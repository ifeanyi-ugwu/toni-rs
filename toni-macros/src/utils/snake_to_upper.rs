...
pub fn snake_to_upper(snake: &Ident) -> Result<String> {
    ...
    if snake_str.starts_with('_') {
        snake_str = snake_str.strip_prefix('_')
        .ok_or_else(|| Error::new(snake.span(), "Failed to strip prefix"))?.to_owned();
    }
    for segment in snake_str.split('_') {
        if segment.is_empty() {
            return Err(Error::new(snake.span(), "Empty segment found in snake case identifier"));
        }
    ...
    for c in upper.chars() {
        if c.is_uppercase() {
            ...
            let lower = c.to_lowercase().next().ok_or_else(|| {
                Error::new(Span::call_site(), "Failed to convert uppercase to lowercase")
            })?;
            result.push(lower);
        } else {
            result.push(c);
